...
const SentRequestTable = () => {
  ...
  const handleFetchRequests = async () => {
    ...
    try {
      const response = await fetch(`${API_BASE_URL}/bookings/v2/sent_request/`, {
        method: "POST",
        headers: {
  ...
  const handlePayNowClick = async () => {
    try {
      const res = await fetch(`${API_BASE_URL}/bookings/v2/check_payment_readiness/`, {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
          "Authorization": `Bearer ${token}`
  ...
  const handleConfirmPayment = async () => {
    ...
    try {
      ...
      const res = await fetch(`${API_BASE_URL}/bookings/v2/create_razorpay_order/`, {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
          "Authorization": `Bearer ${token}`
      ...
      const options = {
        ...
        handler: async function (response) {
          try {
            const verifyRes = await fetch(`${API_BASE_URL}/bookings/v2/verify_razorpay_payment/`, {
              method: "POST",
              headers: {
                "Content-Type": "application/json",
                "Authorization": `Bearer ${token}`
